...
function keyboardEvent()
{
	...
	for(var i = 0; i < playerNum; i ++)
	{
		...
		if(tanks[i].name == 1)
		{
			if(key[K_W]) {player1.move(UP);}
			else if(key[K_S]) {player1.move(DOWN);}
			else if(key[K_A]) {player1.move(LEFT);}
			else if(key[K_D]) {player1.move(RIGHT);}
		  // change shot key 01142016
			if(key[K_F] || key[K_J]) {player1.shot();}
		...
		{
			if(key[K_UP]) {player2.move(UP);}
			else if(key[K_DOWN]) {player2.move(DOWN);}
			else if(key[K_LEFT]) {player2.move(LEFT);}
			else if(key[K_RIGHT]) {player2.move(RIGHT);}

			//if(key[K_SPACE/ENTER]) {player2.shot();}
			if(key[K_SPACE] || key[K_ENTER]) {player2.shot();}
		}
	}
}
